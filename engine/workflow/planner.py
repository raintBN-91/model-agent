...
1. **claude_skill** — 调用一个 Claude Code Skill 执行复杂任务（需要文件操作、代码执行、多步推理等）。**仅当任务确实需要工具链操作时使用**，启动较慢（30-60s）。
2. **llm_tool** — 调用一个 LangChain 工具执行单一操作。**适合所有简单任务**：查询、搜索、问候、解释、输出文本等不需要工具链操作的场景。
...
4. **默认优先使用 llm_tool 类型**，仅当任务需要文件操作、代码执行、模型适配验证、性能调优等工具链操作时才用 claude_skill
