...
| 4.1 | 产品代码不要包含调试入口点 | 调试与隐私 | 中 |
| 4.2 | 正式代码不包含个人隐私信息 | 调试与隐私 | 中 |
| 4.3 | 禁止包含未公开的公网地址 | 调试与隐私 | 中 |
| 4.4 | 禁止在界面、日志中暴露不必要信息 | 调试与隐私 | 中 |
| 5.1 | 创建文件时指定合适权限 | 文件操作 | 中 |
| 5.2 | 外部文件路径必须校验和规范化 | 文件操作 | 高 |
| 5.3 | 禁止使用 tempfile.mktemp | 文件操作 | 高 |
| 5.4 | 临时文件使用完毕及时删除 | 文件操作 | 中 |
| 5.5 | 解压文件必须进行安全检查 | 文件操作 | 高 |
| 6.1 | 禁止使用 pickle.load 加载外部数据 | 序列化 | 高 |
| 6.2 | 禁止序列化未加密的敏感数据 | 序列化 | 高 |
| 6.3 | 禁止使用 yaml.load 函数 | 序列化 | 高 |
| 6.4 | 禁止使用 jsonpickle 模块 | 序列化 | 高 |
| 7.1 | 禁止使用 eval 和 exec 执行不可信代码 | 命令执行 | 高 |
| 7.2 | 禁止使用 OS 命令解析器调用系统命令 | 命令执行 | 高 |
| 7.3 | 避免在命令解析器中使用通配符 | 命令执行 | 中 |
| 7.4 | 禁止使用 subprocess 的 shell=True | 命令执行 | 高 |
| 8.1 | 外部数据禁止使用 .format() 格式化 | 格式化 | 高 |
| 8.2 | 禁止直接使用外部数据拼接 XML | XML安全 | 高 |
| 8.3 | 禁止解析不可信的 XML 实体 | XML安全 | 高 |
| 9.1 | 禁止直接使用外部数据记录日志 | 日志安全 | 中 |
| 9.2 | 禁止在生产代码中使用 assert | 日志安全 | 中 |
| 10.1 | 敏感对象发送出信任区域前签名加密 | 密码学 | 高 |
| 10.2 | 安全场景必须使用安全随机数 | 密码学 | 高 |
| 10.3 | 必须使用 SSLSocket 进行安全数据交互 | 网络安全 | 高 |
