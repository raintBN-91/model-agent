...
基于算子规格信息，自动生成 PyTorch golden 参考实现及完整验证代码。生成的 golden 脚本用于开发阶段快速验证算子实现的正确性，可以作为独立模块被 `test_{op}.py` 等其他脚本导入调用。基于固定模板 [templates/golden-template.py](references/templates/golden-template.py) 生成（在§9 生成文件结构阶段读取该模板）。纯 torch 实现，禁止引入 pypto。
...
生成的文件遵循固定模板 [`references/templates/golden-template.py`](references/templates/golden-template.py)，在生成时读取模板并将占位符替换为实际算子内容。模板包含以下结构：
