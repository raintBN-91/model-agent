# Issue #1035: [Bug]: 在昇腾上使用api方式部署Qwen2-Audio-7B-Instruct模型，openai回复：{"object":"error","message":"The model does not support Transcriptions API","type":"BadRequestError","param":null,"code":400}

## 基本信息

- **编号**: #1035
- **状态**: closed
- **链接**: https://github.com/vllm-project/vllm-ascend/issues/1035
- **创建时间**: 2025-05-30T08:38:37Z
- **关闭时间**: 2025-07-13T09:42:57Z
- **更新时间**: 2025-07-13T09:42:57Z
- **提交者**: @winni0
- **评论数**: 1
...
| 0                         | 0000:41:00.0  | 0           0    / 0          3354 / 65536         |
+===========================+===============+====================================================+
| 7     910B3               | OK            | 82.4        39                0    / 0             |
| 0                         | 0000:42:00.0  | 0           0    / 0          3355 / 65536         |
+===========================+===============+====================================================+
+---------------------------+---------------+----------------------------------------------------+
...
1、执行
```
vllm serve /nfs/models/Qwen2-Audio-7B-Instruct --dtype bfloat16 --max_model_len 8192 --max-num-batched-tokens 8192
```
2、使用openai方式进行通信，代码如下：
```
from openai import OpenAI
import httpx

client = OpenAI(api_key='0', base_url="http://XXX:8000/v1",
                http_client=httpx.Client(proxy="http://XXX:1084")
                )
#
# model_name = client.models.list().data[0].id
mary_had_lamb = r"C:\Users\12235\Downloads\audio-1.mp3"


audio_file = open(mary_had_lamb, "rb")
transcription = client.audio.transcriptions.create(
  model="/nfs/models/Qwen2-Audio-7B-Instruct",
  file=audio_file,
  response_format="text"
)
print(transcription)
```
3、输出：{"object":"error","message":"The model does not support Transcriptions API","type":"BadRequestError","param":null,"code":400}
